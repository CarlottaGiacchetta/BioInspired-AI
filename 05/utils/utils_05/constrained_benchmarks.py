...
class SphereConstrained(ConstrainedBenchmark):
    def __init__(self, dimensions=2, bounder = -5.12):
        Benchmark.__init__(self, dimensions)
        self.b = bounder
        self.bounder = ec.Bounder([bounder] * self.dimensions, [bounder] * self.dimensions)
        self.maximize = False
        self.global_optimum = [0 for _ in range(self.dimensions)]
    
    def generator(self, random, args):
        return [random.uniform(self.b, -self.b) for _ in range(self.dimensions)]
    ...
    def g1(self, x, y):
        return x**2 + y**2 - 1  # Vincolo: x1^2 + x2^2 - 1 <= 0
    
    def g2(self, x, y):
        # Vincolo: x1 + x2 - 1 <= 0 (somma delle coordinate non deve superare 1)
        return x + y - 1
